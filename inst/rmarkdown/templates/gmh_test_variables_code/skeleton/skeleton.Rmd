...
Test of independence should be robust to differences between subsets, so this has been included for the data set overall in the `GMH_test_variables` Rmarkdown.
...
pwc <- dat.clean %>% 
  group_by(!!sym(PREDICTOR[2])) %>%
  emmeans_test(as.formula(paste(OUTCOME, PREDICTOR[1], sep = " ~ ")), p.adjust.method = "bonferroni") 
pwc
```
...
pwc <- dat.clean %>%
  group_by(!!sym(TIME)) %>%
  pairwise_t_test(as.formula(paste(OUTCOME,PREDICTOR,sep = " ~ ")), p.adjust.method = "bonferroni")
pwc
...
pwc2 <- dat.clean %>%
  group_by(!!sym(PREDICTOR)) %>%
  pairwise_t_test(as.formula(paste(OUTCOME,TIME,sep = " ~ ")), p.adjust.method = "bonferroni")
pwc2
```
