...
It is important to confirm if our variablse are independent of each other. If any are significantly correlated this will affect how the variables should be used in following tests. For example, Observed richness and FaithPD are generally correlated, so we should not include both as outcome in a manova test or maybe only one should be tested. It might also be considered to perform an ANCOVA test where we correct for the influence of the correlated variable.
