...
#' @param INDECES Which alpha diversity metrics to calculate. Default c("Observed","Chao1","FaithPD","Shannon")
...
calculate_alpha_diversity <- function(pobject, ntables=100, depth = round(min(sample_sums(pobject))*0.9), INDECES=c("Observed","Chao1","FaithPD","Shannon"), seedstart=500, verbose=FALSE) {
  ...
  if (sum(grepl("FaithPD", INDECES)) > 0) require("picante")
  ...
  for (i in seq(length(INDECES))){
    Alpha_diversity[,INDECES[i]] <- numeric(length = nrow(otu.tab))
    Alpha_diversity[,paste0(INDECES[i],"_sd")] <- numeric(length = nrow(otu.tab))
  }
